...
public class SampleController3 {
	...
	public String doD(Model model, MemberVO vo /* @ModelAttribute("userid") String userid*/) {
		...
		logger.info(" doD() 메서드 호출 -> itwillMember.jsp 뷰 호출");
		
		// 객체 정보를 전달
		// => Model 객체 (스프링에서 제공하는 컨트롤러의 정보를 뷰페이지로 전달하는 객체)
		/*
		 * MemberVO vo = new MemberVO(); vo.setUserid("admin"); vo.setUserpw("1234");
		 * vo.setUsername("관리자"); vo.setUseremail("admin@admin.com");
		 */
		
		// 모델 객체에 정보를 저장 (연결된 뷰페이지로 전달)
		//model.addAttribute("vo", vo);
		model.addAttribute(vo);
		// -> 이름이 없는 경우 전달되는 객체의 클래스명을 사용(첫글자 소문자)
	...
	/*
	 * @RequestMapping("/MemberJoin") public String MemgerJoinAction() {
	 * 
	 * return "main"; }
	 */
}
